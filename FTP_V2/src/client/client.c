		...
		if(ret > 0)
		{
			for(i = 0;i < ret;i++)
			{
				...
				if(evs[i].events == EPOLLIN && evs[i].data.fd == 0)
				{
					...
					ret1 = send(sfd,comm,sizeof(comm),0);
					if(ret1 == -1)
					{
						perror("send");
						return -1;
				...
				if(evs[i].events == EPOLLIN || evs[i].data.fd == sfd)
				{
					switch(comm->key)
					{
						case 1:ls_func(sfd);break;
						case 2:cd_func();break;
						case 3:pwd_func();break;
						case 4:rm_func();break;
						case 5:puts_func();break;
						case 6:gets_func();break;
					}
				}
